...
plot_boxplot(data_clean, by = "Personal Loan")
...
try <- plot_cats%>%partition(cluster)%>%
                  count(personal_loan, Variables,Value )%>%
                  collect()
...
try_clust_cat <- plot_clust_cat%>%partition(cluster)%>%
                                  count(cluster, Variable, Value)%>%
                                  collect()
...
predicted.classes.test.response <- model_prune%>%predict(test_new_level, type = "class")
